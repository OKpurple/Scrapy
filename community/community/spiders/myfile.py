...
class CommunitySpider(scrapy.Spider):
    ...
    def start_requests(self):
        for i in range(1, 2, 1):
            yield scrapy.Request("http://www.clien.net/cs2/bbs/board.php?bo_table=park&page=%d" % i,
                                 self.parse_clien)  # 두번째 변수는 파서
    ...
    def parse_clien(self, response):
        ...
        for sel in response.xpath('//html/body/div[2]/div/div[1]/div[5]/div/div'):
            ...
            item['url'] = 'http://www.clien.net' + \
                          sel.xpath('div/div[@class="list-title"]/a[@class="list-subject"]/@href').extract()[0]

            # 날짜 들고오기
            dateTmp = datetime.strptime(
                sel.xpath('div/div[@class="list-time"]/span/span[@class="timestamp"]/text()').extract()[0],
                "%Y-%m-%d %H:%M:%S")
